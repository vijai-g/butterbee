...
type Action = { type: "ADD"; product: Product; qty?: number } | { type: "REMOVE"; id: string } | { type: "SET_QTY"; id: string; qty: number } | { type: "CLEAR" };
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case "ADD": {
      ...
      let items: CartItem[];
      if (existing) {
        items = state.items.map(i => i.product.id === action.product.id ? { ...i, qty: i.qty + qty } : i);
    ...
    case "SET_QTY":
      return { items: state.items.map(i => i.product.id === action.id ? { ...i, qty: Math.max(1, action.qty) } : i) };
    case "CLEAR":
      toast("Cart cleared");
      return { items: [] };
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  if (JSON.stringify(state) !== JSON.stringify(persisted)) setPersisted(state);
  const value = useMemo(() => {
