  ...
  return (
    <section>
      ...
      {state.items.length === 0 ? (
        <p>
          Your cart is empty.{" "}
          <Link href="/menu" className="underline">Browse the menu</Link>.
        </p>
      ) : (
        <div className="grid gap-6 md:grid-cols-[1fr,360px]">
          ...
          <aside className="card p-4 h-fit sticky top-24">
            ...
            <p className="text-sm text-black/70">Delivery calculated at checkout.</p>
            <p className="mt-2 text-xl font-bold">Total: ₹{total}</p>
            <Link href="/checkout" className="btn btn-primary mt-4 w-full text-center">Go to Checkout</Link>
          </aside>
        </div>
      )}
    </section>
